				...
				<div class="goods-list">
					<div v-for="(good, index) in category.goods" :key="index" class="goods">
						...
						<div class="goods-content">
							...
							<div class="tag">
								<image src="../../static/image/商品标签.png"
									style="height: 24rpx; width: 24rpx;margin-right: 15rpx;"></image>{{ good.tag }}
							</div>
							<div class="intro">{{ good.intro }}</div>
							<div class="price-button">
								<div class="price">¥ {{ good.price }} <text
										style="font-size: 10px;margin-left: 8rpx;">起</text></div>
			...
			calculateCategoryHeights() {
				this.$nextTick(() => {
					...
					this.categories.forEach(category => {
						...
						const goodsHeight = category.goods.length *
							230; // 每个商品 200rpx + margin-bottom 30rpx
...
<style>
	...
	.category-item {
		...
		/* 文本大小 */
		padding: 35rpx 20rpx;
		/* 30rpx 顶部和底部的 padding，15rpx 左侧和右侧的 padding */
		cursor: pointer;
		/* 鼠标指针样式为手型 */
	}
	...
	.category-item:before {
		...
		/* 将竖线向上偏移自身高度的一半，以使其居中 */
		height: 70%;
		/* 竖线的高度为容器高度的70%，可以根据需要调整 */
		width: 3px;
		/* 竖线的宽度 */
